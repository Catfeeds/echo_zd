<?php
$this->pageTitle = $this->controllerName.'编辑列表';
...
<table class="table table-bordered table-striped table-condensed flip-content">
    ...
    <tbody>
    <?php foreach($infos as $v): $user = $v->user; ?>
        <tr>
            <td  class="text-center"><?php echo $v->id ?></td>
            <td  class="text-center"><?php echo $v->plot?$v->plot->title: ''?></td>
            <td  class="text-center"><?php echo '<a target="_blank" href="'.$this->createUrl('/api/index/detail',['id'=>$v->hid]).'">查看楼盘</a>'.' '.'<a target="_blank" href="edit?id='.$v->hid.'">编辑楼盘</a>'?></td>
            <td  class="text-center"><?php echo ($user?$user->name:'').' '.($user?$user->phone:'') ?></td>
