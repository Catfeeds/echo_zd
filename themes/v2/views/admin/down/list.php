   ...
   <table class="table table-bordered table-striped table-condensed flip-content table-hover">
    ...
    <?php foreach($infos as $k=>$v): $user = $v->user;$plot = $v->plot; ?>
        <tr>
            ...
            <td class="text-center"><a target="_blank" href="/api/index/detail?id=<?=$v->hid?>"><?=$plot?$plot->title:''?></a></td>
            ...
            <td class="text-center"><?php echo CHtml::ajaxLink(PlotDownExt::$status[$v->status],$this->createUrl('changeStatus'), array('type'=>'get', 'data'=>array('id'=>$v->id,'class'=>get_class($v)),'success'=>'function(data){location.reload()}'), array('class'=>'btn btn-sm '.UserExt::$statusStyle[$v->status])); ?></td>
