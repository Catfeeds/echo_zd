 ...
 * @date(2017.2.12)
 */
class PlotCallExt extends PlotCall{
	/**
     * 定义关系
     */
    ...
    {
        if($plot = $this->plot) {
            $call_num = Yii::app()->db->createCommand("select count(id) from plot_call where hid=".$plot->id)->queryScalar();
            Yii::app()->db->createCommand("update plot set call_num=$call_num where id=".$plot->id)->execute();
        }
        parent::afterSave();
    }
