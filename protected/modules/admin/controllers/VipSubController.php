 ...
 * 集客控制器
 */
class VipSubController extends AdminController{
	...
	public $modelName = 'VipSubExt';

	public function init()
	{
		parent::init();
		$this->controllerName = '集客';
	...
	public function actionList($type='title',$value='',$time_type='created',$time='',$cate='')
	{
		...
		if($value = trim($value))
            ...
            } elseif ($type=='vip_phone') {
            	$criteria->addSearchCondition('vip_phone', $value);
            } elseif ($type=='user_phone') {
            	$criteria->addSearchCondition('user_phone', $value);
            }
        //添加时间、刷新时间筛选
        if($time_type!='' && $time!='')
        {
